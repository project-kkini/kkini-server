...
import com.server.ggini.domain.restaurant.service.RestaurantCreateService;
...
public class ReviewFirstCreateService {
    ...
    @Transactional
    public Long createReviewFirst(Long seoulRestaurantId, ReviewPostRequest request, Member member) {
        needsTagRepository.existsByIdsElseThrow(request.needsTagIds());
        priceTagRepository.existsByIdsElseThrow(request.priceTagIds());

        Restaurant restaurant = restaurantCreateService.createRestaurant(seoulRestaurantId, request.needsTagIds(),
                request.priceTagIds());
        reviewRepository.save(request.toEntity(member, restaurant, true));
